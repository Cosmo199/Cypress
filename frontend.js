...
context('front-end script test workflows', () => {
   ...
   function login () {
    ...
    it ('success register',()=>{
      cy.visit('http://www.ploichao.com/backend-user/register')
      cy.get('[name="name"]').type('aisbot01').should('have.value', 'aisbot01')
      ...
      cy.contains("บันทึกข้อมูลเรียบร้อย")
      if (cy.contains("สมัครสมาชิกเรียบร้อย")) {
        cy.log('สมัครสมาชิกสำเร็จแล้ว')
      } else{
        cy.log('summit code error')
      }
